...
const MicIcon = React.memo(
  (props: React.ComponentProps<typeof Microphone>) => <Microphone {...props} />
);
const StopIcon = React.memo(
  (props: React.ComponentProps<typeof Square>) => <Square {...props} />
);
const SendIcon = React.memo(
  (props: React.ComponentProps<typeof ArrowUp>) => <ArrowUp {...props} />
);
    ...
    <AnimatePresence>
      {active && (
        <motion.span
          ...
          animate={{
            opacity: 1,
            // derive scale without re-rendering the parent
            // 1 + volume * 1.5
            scale: scale,
          }}
...
}, (a, b) => a.mode === b.mode && a.strokeWidth === b.strokeWidth && a.className === b.className);
...
export const ActionButton: React.FC<ActionButtonProps> = ({
  ...
  const tooltip = React.useMemo(() => {
    ...
    if (mode === "busy") return (t as any)?.("tooltipBusy") ?? "Working…";
  ...
  const cancelLabel = t("cancelRecording", { default: "Cancel recording" }) as any;
